...
namespace Ngram
{
    public class Ngram
    {
        ...
        private string corpusPath = @"C:\Users\Kiet\OneDrive\Thesis\NewCorpus";
        ...
        public void generateUnigram()
        {
            ...
            string[] getFile = Directory.GetFiles(corpusPath, "*.txt", SearchOption.AllDirectories);
        ...
        public void readUni(string path)
        {
            ...
            foreach (string line in uniGram)
            {
                ...
                {
                    //Console.WriteLine(_uniPos.Count + " " + count +line);
        ...
        public void generateBigram()
        {
            ...
            foreach (string file in getFile)
            {
                ...
                foreach (string phrase in phraseArr)
                {
                    string newPhrase = String.Format("{0} {1} {2}", "<s>", phrase.Trim(), "</s>");
                    string[] words = newPhrase.Split(' ');
            ...
            count = 1;
            foreach (KeyValuePair<string, int> pair in biAmount)
            {
                count++;
                if (pair.Value > 10)
                {
                    string[] sylls = pair.Key.Split(' ');
                    ...
                    output.AppendFormat("{0} {1} {2} {3}", sylls[0], sylls[1], pair.Value, "\n");
                    if (count % 1000 == 0 || count == biAmount.Count)
                        Console.WriteLine(string.Format("Converting bigram: {0}/{1}", count, biAmount.Count));
                }
            }

            File.WriteAllText(@"C:\Users\Kiet\OneDrive\Thesis\Ngram\bi.txt", output.ToString());
        }
        //đọc file bigram
